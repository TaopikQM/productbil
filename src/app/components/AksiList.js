...
const AksiList = ({ navigateToReviewPage}) => {
  ...
  useEffect(() => {
    const placesRef = ref(rtdb, 'aksi_B');
  ...
  const toggleStatus = (id, currentStatus) => {
    ...
    update(ref(rtdb, `aksi_B/${id}`), { status: newStatus });
  };

  const deletePlace = (id) => {
    const placeRef = ref(rtdb, `aksi_B/${id}`);
    remove(placeRef);
  };
  ...
  const saveEdit = async () => {
    const placeRef = ref(rtdb, `aksi_B/${selectedPlaceId}`);
    const updates = { ...newData };
    if (newData.images) {
      const imageUrls = await Promise.all(
        Array.from(newData.images).map(async (image) => {
          const imageRef = storageRef(storage, `aksi_B/${selectedPlaceId}/${image.name}`);
