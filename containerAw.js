...
export const ContainerAw = ({ children, isDayMode }) => {
  return (
    <div className={`w-full min-h-screen flex justify-center items-start py-10 isDayMode ? 'bg-white text-black' : 'bg-gray-800 text-white`>
      <div className={`w-full max-w-7xl p-10 rounded-lg shadow-lg isDayMode ? 'bg-white text-black' : 'bg-gray-800 text-white`}>
        {children}
      </div>
    </div>
  );
