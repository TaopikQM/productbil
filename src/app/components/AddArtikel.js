...
const AddArtikel = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const newWisataRef = push(databaseRef(rtdb, 'artikel_B'));
      ...
      const imageUrls = await Promise.all(
        images.map(async (image) => {
          const imageRef = storageRef(storage, `artikel_B/${newWisataKey}/${image.name}`);
      ...
      await update(databaseRef(rtdb, `artikel_B/${newWisataKey}`), {
        imageUrls
      });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label htmlFor="name" className="block text-sm font-medium text-gray-700">Nama Artikel</label>
        <input
          type="text"
          id="name"
          value={name}
