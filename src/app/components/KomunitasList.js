...
const ArtikelList = ({ navigateToReviewPage}) => {
  ...
  useEffect(() => {
    const placesRef = ref(rtdb, 'komunitas_B');
  ...
  const toggleStatus = (id, currentStatus) => {
    ...
    update(ref(rtdb, `komunitas_B/${id}`), { status: newStatus });
  };

  const deletePlace = (id) => {
    const placeRef = ref(rtdb, `komunitas_B/${id}`);
    remove(placeRef);
  };
  ...
  const saveEdit = async () => {
    const placeRef = ref(rtdb, `komunitas_B/${selectedPlaceId}`);
    const updates = { ...newData };
    if (newData.images) {
      const imageUrls = await Promise.all(
        Array.from(newData.images).map(async (image) => {
          const imageRef = storageRef(storage, `komunitas_B/${selectedPlaceId}/${image.name}`);
        ...
        <thead>
          <tr>
           <th className="px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Media</th>
            ...
            <th className="px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Keterangan</th>
            ...
            <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
              <div>
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                  <h3 className="text-lg leading-6 font-medium text-gray-900">Edit Aksi</h3>
                  <div className="mt-2">
                    <div>
