...
const AddKomunitas = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const newWisataRef = push(databaseRef(rtdb, 'komunitas_B'));
      ...
      const imageUrls = await Promise.all(
        images.map(async (image) => {
          const imageRef = storageRef(storage, `komunitas_B/${newWisataKey}/${image.name}`);
      ...
      await update(databaseRef(rtdb, `komunitas_B/${newWisataKey}`), {
        imageUrls
      });
