...
const AddProduct = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const newWisataRef = push(databaseRef(rtdb, 'product_B'));
      ...
      const imageUrls = await Promise.all(
        images.map(async (image) => {
          const imageRef = storageRef(storage, `product_B/${newWisataKey}/${image.name}`);
      ...
      await update(databaseRef(rtdb, `product_B/${newWisataKey}`), {
        imageUrls
      });
